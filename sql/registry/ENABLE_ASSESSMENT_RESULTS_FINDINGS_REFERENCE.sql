-- Source 2 sources_source Assessment Results finding-reference registry extension
-- Date: 2026-09-17
-- Purpose: add the canonical results[].findings[] collection without touching existing AR registry rows.
-- This is idempotent and duplicate-guarded. It changes registry metadata only; no DIM/FACT DML.

EXECUTE IMMEDIATE $$
DECLARE
  tx NUMBER;
  conflicts NUMBER;
  duplicates NUMBER;
  verified NUMBER;
  started BOOLEAN DEFAULT FALSE;
  committing BOOLEAN DEFAULT FALSE;

  active_tx EXCEPTION (-20701, 'AR_FINDINGS_ACTIVE_TRANSACTION: finish the existing transaction first.');
  conflict EXCEPTION (-20702, 'AR_FINDINGS_REGISTRY_CONFLICT: inspect the existing finding path; no changes accepted.');
  verification_failed EXCEPTION (-20703, 'AR_FINDINGS_REGISTRY_VERIFICATION_FAILED: changes were not accepted.');
  commit_unknown EXCEPTION (-20704, 'AR_FINDINGS_COMMIT_UNCONFIRMED: inspect registry before retrying.');
BEGIN
  SELECT CURRENT_TRANSACTION() INTO :tx;
  IF (tx IS NOT NULL) THEN RAISE active_tx; END IF;

  -- Parent must already be the accepted Source-record collection.
  SELECT COUNT(*) INTO :verified
  FROM RTX_RAW_DEV.ES_ESC_GRC.OSCAL_ELEMENT_REGISTRY
  WHERE UPPER(TRIM(OSCAL_MODEL_KEY)) = 'ASSESSMENT_RESULTS'
    AND TRIM(NODE_PATH) = 'assessment-results.results[]'
    AND TRIM(PARENT_NODE_PATH) = 'assessment-results'
    AND IS_COLLECTION = TRUE
    AND TRIM(INSTANCE_KEY_RULE) = 'SOURCE_RECORD_ID'
    AND COALESCE(TRIM(OPERATOR), '') = 'record'
    AND IS_ACTIVE = TRUE;
  IF (verified <> 1) THEN RAISE verification_failed; END IF;

  SELECT COUNT(*) INTO :duplicates
  FROM (
    SELECT TRIM(NODE_PATH) AS NODE_PATH
    FROM RTX_RAW_DEV.ES_ESC_GRC.OSCAL_ELEMENT_REGISTRY
    WHERE TRIM(NODE_PATH) = 'assessment-results.results[].findings[]'
    GROUP BY TRIM(NODE_PATH)
    HAVING COUNT(*) > 1
  );
  IF (COALESCE(duplicates, 0) > 0) THEN RAISE conflict; END IF;

  SELECT COUNT(*) INTO :conflicts
  FROM RTX_RAW_DEV.ES_ESC_GRC.OSCAL_ELEMENT_REGISTRY
  WHERE TRIM(NODE_PATH) = 'assessment-results.results[].findings[]'
    AND (
      UPPER(COALESCE(TRIM(OSCAL_MODEL_KEY), '')) <> 'ASSESSMENT_RESULTS'
      OR COALESCE(TRIM(PARENT_NODE_PATH), '') <> 'assessment-results.results[]'
      OR COALESCE(TRIM(ELEMENT_TYPE), '') <> 'findings'
      OR COALESCE(IS_COLLECTION, FALSE) <> TRUE
      OR COALESCE(TRIM(INSTANCE_KEY_RULE), '') <> 'CONTENT_ID'
      OR COALESCE(TRIM(ITEM_PATH), '') <> '$'
      OR COALESCE(TRIM(OPERATOR), '') <> 'references'
      OR COALESCE(TRIM(UUID_POLICY), '') <> 'node'
      OR COALESCE(PROCESS_ORDER, -1) <> 5
      OR REQUIRED_MEMBERS IS NOT NULL
    );
  IF (COALESCE(conflicts, 0) > 0) THEN RAISE conflict; END IF;

  BEGIN TRANSACTION;
  started := TRUE;

  MERGE INTO RTX_RAW_DEV.ES_ESC_GRC.OSCAL_ELEMENT_REGISTRY t
  USING (
    SELECT
      'ASSESSMENT_RESULTS' AS OSCAL_MODEL_KEY,
      'assessment-results.results[].findings[]' AS NODE_PATH,
      'findings' AS ELEMENT_TYPE,
      'assessment-results.results[]' AS PARENT_NODE_PATH,
      TRUE AS IS_COLLECTION,
      'CONTENT_ID' AS INSTANCE_KEY_RULE,
      5 AS PROCESS_ORDER,
      TRUE AS IS_ACTIVE,
      '$' AS ITEM_PATH,
      'references' AS OPERATOR,
      'node' AS UUID_POLICY,
      NULL AS REQUIRED_MEMBERS
  ) s
    ON TRIM(t.NODE_PATH) = s.NODE_PATH
  WHEN MATCHED THEN UPDATE SET
      OSCAL_MODEL_KEY = s.OSCAL_MODEL_KEY,
      ELEMENT_TYPE = s.ELEMENT_TYPE,
      PARENT_NODE_PATH = s.PARENT_NODE_PATH,
      IS_COLLECTION = s.IS_COLLECTION,
      INSTANCE_KEY_RULE = s.INSTANCE_KEY_RULE,
      PROCESS_ORDER = s.PROCESS_ORDER,
      IS_ACTIVE = s.IS_ACTIVE,
      ITEM_PATH = s.ITEM_PATH,
      OPERATOR = s.OPERATOR,
      UUID_POLICY = s.UUID_POLICY,
      REQUIRED_MEMBERS = s.REQUIRED_MEMBERS
  WHEN NOT MATCHED THEN INSERT (
      OSCAL_MODEL_KEY, NODE_PATH, ELEMENT_TYPE, PARENT_NODE_PATH,
      IS_COLLECTION, INSTANCE_KEY_RULE, PROCESS_ORDER, IS_ACTIVE,
      ITEM_PATH, OPERATOR, UUID_POLICY, REQUIRED_MEMBERS
  ) VALUES (
      s.OSCAL_MODEL_KEY, s.NODE_PATH, s.ELEMENT_TYPE, s.PARENT_NODE_PATH,
      s.IS_COLLECTION, s.INSTANCE_KEY_RULE, s.PROCESS_ORDER, s.IS_ACTIVE,
      s.ITEM_PATH, s.OPERATOR, s.UUID_POLICY, s.REQUIRED_MEMBERS
  );

  SELECT COUNT(*) INTO :verified
  FROM RTX_RAW_DEV.ES_ESC_GRC.OSCAL_ELEMENT_REGISTRY
  WHERE UPPER(TRIM(OSCAL_MODEL_KEY)) = 'ASSESSMENT_RESULTS'
    AND TRIM(NODE_PATH) = 'assessment-results.results[].findings[]'
    AND TRIM(PARENT_NODE_PATH) = 'assessment-results.results[]'
    AND TRIM(ELEMENT_TYPE) = 'findings'
    AND IS_COLLECTION = TRUE
    AND TRIM(INSTANCE_KEY_RULE) = 'CONTENT_ID'
    AND TRIM(ITEM_PATH) = '$'
    AND TRIM(OPERATOR) = 'references'
    AND TRIM(UUID_POLICY) = 'node'
    AND PROCESS_ORDER = 5
    AND REQUIRED_MEMBERS IS NULL
    AND IS_ACTIVE = TRUE;
  IF (verified <> 1) THEN RAISE verification_failed; END IF;

  committing := TRUE;
  COMMIT;
  started := FALSE;

  RETURN OBJECT_CONSTRUCT(
    'STATUS', 'AR_FINDINGS_REFERENCE_REGISTRY_VERIFIED',
    'PATH', 'assessment-results.results[].findings[]',
    'ACTIVE_ROWS', verified,
    'COMMITTED', TRUE
  );
EXCEPTION
  WHEN OTHER THEN
    IF (committing) THEN RAISE commit_unknown; END IF;
    IF (started) THEN ROLLBACK; END IF;
    RAISE;
END;
$$;
