-- READ ONLY: one-record values preview. This file contains no data writes.
-- 1. Replace every ${jv_raw_table_name} with the ACTUAL fully qualified raw table
--    when using a Snowflake SQL worksheet (Matillion may resolve its existing variable).
-- 2. Replace REPLACE_WITH_REQUESTED_OBJECT_ID below with the raw RequestedObject.Id,
--    NOT the derived CONTENT_ID. Keep it quoted.
-- Includes already-curated rows for inspection ONLY. The production candidate's
-- pending-only UPDATE predicates have NOT changed.
-- This returns real source values: inspect in Snowflake; do not post sensitive
-- payloads, identifiers or unredacted screenshots to GitHub.
-- Existing strict casts/nested extraction are preserved, not silently repaired.
-- This is derived from the posted SQL transcription and is not live-verified.
