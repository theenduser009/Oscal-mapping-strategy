...
model_values = canonical_mapping_pdf["OSCAL_MODEL"].fillna("").str.upper()
canonical_mapping_pdf = canonical_mapping_pdf[
    (model_values == "") | (model_values == CONFIG["OSCAL_MODEL"].upper())
].copy()
...
canonical_mapping_pdf.sort_values(
    ["OSCAL_ELEMENT_PATH", "SOURCE_FIELD_NAME"],
    inplace=True,
    kind="stable",
)
...
for mapping_row in CANONICAL_MAPPING_ROWS:
    element_path = str(mapping_row["OSCAL_ELEMENT_PATH"]).strip()
    MAPPINGS_BY_ELEMENT_PATH.setdefault(element_path, []).append(mapping_row)

print("Canonical SSP mapping rows:", len(CANONICAL_MAPPING_ROWS))
print("Mapped element paths:", len(MAPPINGS_BY_ELEMENT_PATH))
