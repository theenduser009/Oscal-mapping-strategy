...
All **549 local tests pass**. Independent parity checks match the accepted SSP fixture exactly and the AR17 standalone business output. New-field and third-model tests run through the actual shared builder with legacy classifiers unavailable. Cardinality, required/null policy and simple typed/enum/range validation are declarative constraints. Cell Four is roughly half its previous size; this reduction removes duplicate engines, not the shared writer's safeguards.

**Next: inspect the full existing Cell Seven routing report before giving new live run advice.** The owner reported 608 input rows, 459 blocked, 60 deferred and 46 excluded; only a partial spoken report is available. The cited missing-approved-metadata example is deferred under current policy and does not establish the cause of the blocked rows. No new shared-workflow Snowflake result is accepted. The code now excludes clearly other-model rows before field validation and prints exact reason/severity totals with at most 25 samples; unknown/conflicting ownership still blocks. This fixes a reproduced routing defect but does not prove all 459 live blockers resolved. Do not request an unchanged rerun or COMMIT.

Maintain code once in notebooks/cells and generate V2/combined pages with tools/sync_notebook_cells.py. The owner still uses the same seven V2 pages, no extra runtime dependency or cell. The catalog is unchanged. The executable plan is generated from the sheet/registry/catalog; the catalog itself is not automatically synchronized from Excel. [Metadata contract and limitations](../notebooks/metadata/README.md).
