...
| POA&M | `plan-of-action-and-milestones.poam-items[]` | `POAMS` | Reference graph implemented; live preview pending | Owner confirmed same Source One and Reference row [E15]. UUID-only package-scoped nodes and root links; no item-detail hydration. | POAM destination binding configured using SSP/AR pattern; live registry preparation and target-aware preview pending. See [next run](POAM_NEXT_RUN.md). |
